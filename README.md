...
## 📝 Story
학원에서 수업을 들은 후 진행했던 첫 프로젝트로 새로운 것을 구상하기 보다 기존에 잘 나와 있는 페이지를 클론 코딩하는 것이 성장에 더 도움이 될 것이라 생각하여, 음악 스트리밍 서비스 업체인 spotify를 선정하여 프로젝트를 진행하게 되었습니다.

실무에서 많이 쓰이는 부트스트랩과 친숙해지기 위해 부트스트랩 그리드 시스템을 선택하여 레이아웃 배치를 구현했고, pc, tab, mb 사이즈별로 사용자가 편하게 이용할 수 있도록 UI를 재구성하여 미디어 쿼리로 반응형 웹 페이지를 구현해 보았습니다.

spotify는 API를 제공하여 필요한 정보를 요청하여 받아올 수 있었고, 이러한 구현 과정을 통해 API 문서를 읽는 능력을 키우는 데 도움이 되었습니다. 또한 문제 해결을 위해 다른 개발자들의 코드를 보고 이해하며, 그 코드를 내 코드에 적용시키는 방법까지 배울 수 있어 보람을 느낀 프로젝트였습니다.
<br/><br/>
